...
function template(
  ...
  function get(selector) {
    ...
    } else if (data.get(key)) value = accessNestedPropByString(data.get(key), rest);
    return { key, rest, value };
  }
  function split(string, regexes) {
    string = [string];
    for (let regex of regexes) string = string.map((part) => part.split(regex)).flat(Infinity);
    return string;
  }
    ...
    if (parts.length === 1) {
      ...
      if (modifiers.for && Array.isArray(modifiers.for.value)) {
        ...
        for (let elem of modifiers.for.value) {
          ...
          result += template(get(target).value, { components, data: scoped }) || '';
        }
      } else {
  ...
  return parsed
    .map((part) => (part.startsWith('<!--') && part.endsWith('-->') ? part : minify(part)))
    .reverse()
    .join('');
}
