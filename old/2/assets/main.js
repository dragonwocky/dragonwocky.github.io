...
$(document).ready(function() {
  ...
  /* Manage download links. Replaced with alert to download from new site.
  ...
  $('.download, .other-download, .view, .code')
  ...
  function changePage() {
    $('article').each(function() {
      ...
      if (
        $(this).attr('id') === /[^\#]*$/.exec(window.location.href)[0] &&
        ...
        $('title').html(
          capitalizeFirstLetter(/[^\#]*$/.exec(window.location.href)[0]) +
            ' | ' +
            $siteName
        );
      }
      ...
      if (
        ...
        $('#' + /[^\#]*$/.exec(window.location.href)[0]).length === 0
      ) {
  ...
  $('.form .submit').click(function() {
    ...
    for (var i = 0; i < $inputs.length; i++) {
      if ($inputs[i] === '') {
        //Check if all the fields are filed in.
    ...
    if ($filled[0] && $filled[1] && $filled[2] && $filled[3]) {
      //Set the link to the correct mailto url.
      ...
      var $body =
        ...
        $('[name=email]').val() +
        '%0A Message: ' +
        encodeURI($('[name=message]').val());
      var $url =
        'mailto:' +
        $('.form').attr('data-email-to') +
        '?subject=' +
        $subject +
        '&body=' +
        $body;
  ...
  function getYear() {
    ...
    if (year === parseFloat($('.year').html()) || isNaN($('.year').html())) {
      //Check if the year in footer is the current year, or if it is a number.
      return year;
    } else {
